...
- **TeX Live Package Management**: Offers three versions of TeX Live installations for flexible LaTeX environment configurations:
  ...
  - **Slim Version (`slim`)**: Based on the Overleaf repository’s Dockerfile, this version includes a minimal TeX Live installation with only essential packages. (_Community version_)
  - **Base Version (`base`)**: A lightweight version containing only basic dependencies like `wget` and a minimal TeX Live setup. (_Base version_)
